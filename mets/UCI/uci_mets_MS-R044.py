...
def main(argv=None):
   ...
   for file in files:
      if file.endswith('.mets.xml'):
         ...
         thiscollection = 'Pleasants Family Papers'
...
def extract_modsproperties(document):
   ...
   alttitle_items = []
   for titleInfo in mods.iterfind('mods:titleInfo', namespaces=nsmap):
       type = titleInfo.get('type')
       if type == 'alternative':
           for title in titleInfo.iterfind('mods:title', namespaces=nsmap):
               alttitle_items.append(title.text)
       else:
           for title in titleInfo.iterfind('mods:title', namespaces=nsmap):
